...
%%%==============================================================================
\documentclass[article,nofontwarning,nogeometry,english,tocdepth=3,secdepth=3]{ufrgscca} % tcbundle  tcpack tcmain
...
\iffulldoc
\tstitle{
  author={Alceu Frigeri\footnote{\tsfmt{\url{https://github.com/alceu-frigeri/ufrgscca}}} },
...
\ifversiontest
\ExplSyntaxOn
\clist_map_inline:nn {ufrgscca,ufrgscca-abnt,ufrgscca-core,ufrgscca-cover,ufrgscca-lists,ufrgscca-forms,ufrgscca-coord,ufrgscca-curr,ufrgscca-ppc}{\PkgInfoDescription{#1}}
\ExplSyntaxOff
\fi
...
\begin{codedescribe}{\NewChapListEnv}
	...
	\end{codesyntax}
\end{codedescribe}
Creates a \emph{chapter like} lists, e.g. `List of Symbols' or `List of acronyms'. With it, a new environment is created, \tsobj[meta]{envname}, with an associated `numberless' chapter name \tsobj[meta]{displayname}. The newly created environment will implement a \emph{description} like environment (thanks to \tsobj[pkg]{enumitem}) with an optional and a mandatory argument (see below).
...
\begin{codedescribe}[code,new=2023/11/18,update=2024/04/18]{\DataFields,\eDataSet,\eDataFields,\DataGet}
	...
	\end{codesyntax}
These are, respectively, \tsobj{\starray_get_prop:nn, \starray_term_parser:n,\starray_parsed_get_prop:n} and \tsobj{\starray_get_prop:nnN} from \tsobj[pack]{starray}. One can reference/get any field from the main \tsobj[pack]{starray} defined structures: \tsobj[key]{student,activity} as defined at chapter \ref{activity-def}.
\end{codedescribe}
...
\end{codedescribe}
\begin{describelist}{option}
\describe{tccI}{Generate 3 pages, a first cover one, a second with work's description and a third last one with work's approval for TCC-I}
\describe{tccII}{Generate 3 pages, a first cover one, a second with work's description and a third last one with work's approval for TCC-II}
\describe{internship}{Generate 2 pages, a first cover one and a second with work's approval for internship report}
\describe{internship-opt}{Generate 2 pages, a first cover one and a second with work's approval for optional internship report}
...
\begin{codedescribe}[code,new=2023/11/18,update=2025/09/18]{\csummary,\classremark,\bibdef}
    \begin{codesyntax}%
        \tsmacro{\csummary}{desc}
        \tsmacro{\classremark}{remark}
        \tsmacro{\bibdef}[type]{bibref}
    \end{codesyntax}
...
\begin{codedescribe}[code,update=2023/11/18]{\addclass,\depdef,\altdep}
    \begin{codesyntax}%
        \tsobj[code]{\addclass}\tsargs[oarg]{color}\tsargs[marg]{pos,classID,kind}\tsargs[oarg]{obs}
        \tsobj[code]{\depdef}\tsverb[oarg]{*}\tsargs[oarg]{pos}\tsargs[marg]{classID}
        \tsmacro{\altdep}{}
    \end{codesyntax}
\tsmacro{\addclass}{} adds a class, \tsobj[marg]{classID}, to the current semester. \tsobj[marg]{pos} will set the vertical position of this class (when using \tsobj{\GraphEtp}. \tsobj[marg]{kind} will set the class kind and \tsobj[oarg]{obs} will add an observation.
